...
class AzureSubscriptionForm(NetBoxModelForm):
    ...
    comment = CommentField()
    tenant = DynamicModelChoiceField(queryset=models.AzureTenantModel.objects.all())
...
class AzureSubscriptionFilterForm(NetBoxModelFilterSetForm):
    ...
    tenant = forms.ModelMultipleChoiceField(
        queryset=models.AzureTenantModel.objects.all(), required=False
    )
    status = forms.MultipleChoiceField(
        choices=choices.SubscriptionStatusChoices, required=False
    )
